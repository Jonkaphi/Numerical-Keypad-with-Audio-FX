...
void populat_HID_fast_media_controls_report(IO_controls_handler keyboard_inst){//[volatile uint8_t * bttn_controls, volatile int8_t * encoder_val, int8_t * HID_data_packet, uint8_t * new_packet_state){
    ...
    //prev_bttn_controls = keyboard_inst->controls;
    keyboard_inst->controls = (keyboard_inst->controls&~(0x01));
    keyboard_inst->enc_count = 0;

    //reset the mute logical input for the that usage 
    //rewrite the of the mute bit is done so that it sends the report again but with a 0, 
    //as to follow the expected pattern for control of 1 then 0 to mute and umute is to send another 1 then 0
    //keyboard_inst->controls = (keyboard_inst->controls&~(0x01));

    // if((uint32_t)HAL_GetTick()-last_tick>=600){
    //     keyboard_inst->controls = (keyboard_inst->controls&(0x01));
