...
int main(void)
{
 ...
 RCC->APB2ENR |= RCC_APB2ENR_DBGMCUEN; //enables suspension of the APB2 bus dduring debug
  ...
  while (1)
  {
    ...
    populate_HIDreport(&keyboard.curr_keycode_arry,&keyboard.prev_keycode_arry, &keyboard.keys_HID_data_report, &keyboard.packet_update_flag, keyboard.highest_element );
    
    if(keyboard.packet_update_flag){
	  USBD_HID_SendReport(&hUsbDeviceFS,&keyboard.keys_HID_data_report,sizeof(keyboard.keys_HID_data_report));
